...
export class PostgresRepository implements RegionfinderRepository {
  ...
  async stopTile(z: number, x: number, y: number, modes: string[] = []): Promise<Buffer | null> {
    return this.mvtTile(
      `
      WITH bounds AS (
      ...
      mvtgeom AS (
        SELECT sp.public_id,
               ...
               sp.state_code,
               sp.modes,
               ST_AsMVTGeom(ST_Transform(sp.geometry, 3857), bounds.geom) AS geom
        FROM stop_places sp
        JOIN data_snapshots snap ON snap.id = sp.snapshot_id AND snap.is_active = true
        CROSS JOIN bounds
    ...
    return this.mvtTile(
      ...
      mvtgeom AS (
        SELECT rp.id::text,
               ...
               END AS route_color,
               rp.geometry_quality,
               ST_AsMVTGeom(ST_Transform(rp.geometry, 3857), bounds.geom) AS geom
        FROM route_patterns rp
        JOIN routes r ON r.id = rp.route_id AND r.snapshot_id = rp.snapshot_id
        JOIN data_snapshots snap ON snap.id = rp.snapshot_id AND snap.is_active = true
        CROSS JOIN bounds
        WHERE rp.geometry IS NOT NULL
          AND ST_Intersects(ST_Transform(rp.geometry, 3857), bounds.geom)
  ...
  private async mvtTile(sql: string, z: number, x: number, y: number, modes: string[] = []): Promise<Buffer | null> {
    const result = await this.pool.query<{ tile: Buffer | null }>(sql, [z, x, y, modes])
